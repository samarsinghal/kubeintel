"""
Tests for Kubernetes tools for AWS Strands integration.
"""
...
class TestKubernetesTools(unittest.TestCase):
    """Test cases for Kubernetes tools."""
    ...
    def test_get_pods_with_label_selector(self, mock_client_class):
        ...
        self.assertEqual(result["label_selector"], "app=web")
        self.assertEqual(result["summary"]["running"], 1)
    ...
    @patch('src.strands.kubernetes_tools.KubernetesClient')
    def test_get_pods_with_multiple_label_selectors(self, mock_client_class):
        """Test get_pods with multiple label selectors."""
        # Configure the mock
        mock_client_class.return_value = self.mock_k8s_client
        ...
        # Call the method
        result = tools.get_pods(label_selector="app=web,tier=frontend")
        
        # Verify the result
        self.assertEqual(result["count"], 1)
        self.assertEqual(result["label_selector"], "app=web,tier=frontend")
        self.assertEqual(len(result["pods"]), 1)
        self.assertEqual(result["pods"][0]["name"], "pod-1")
    ...
    @patch('src.strands.kubernetes_tools.KubernetesClient')
    def test_get_pods_with_invalid_label_selector(self, mock_client_class):
        """Test get_pods with invalid label selector."""
        # Configure the mock
        mock_client_class.return_value = self.mock_k8s_client
        ...
        # Call the method with a non-existent label
        result = tools.get_pods(label_selector="nonexistent=value")
        
        # Verify the result
        self.assertEqual(result["count"], 0)
        self.assertEqual(result["label_selector"], "nonexistent=value")
